...
# ===============================================
# New helpers for x,y-free feature calibration
# ===============================================
AP_SUFFIX = "_rssi"
...
def _total_power_db(rssis: Sequence[Optional[float]]) -> Optional[float]:
    mws = [_dbm_to_mw(v) for v in rssis if v is not None]
    if not mws:
        return None
    return 10.0 * math.log10(sum(mws))

def fit_xy_free_calibration(pre_docs, ap_labels):
    """
    Fit x,y-free calibration for two features:
      - <label>_rssi_rssi_1m  := RSSI - P95(RSSI for that AP)
      - <label>_residual      := RSSI - (intercept[label] + gamma * total_power_db)

    Uses only observed RSSIs from pre_docs (no coordinates / distances).
    Returns dict with keys: ap_p95 (dict), ap_intercepts (dict), gamma (float)
    """
    per_ap_values = defaultdict(list)
    rows, ys = [], []

    for d, label_best in pre_docs:
        # Collect per-AP RSSI values for percentiles
        for lbl, rec in label_best.items():
            r = rec.get("RSSI")
            if r is not None:
                per_ap_values[lbl].append(float(r))

        # Build residual-model rows only when we have all AP RSSIs present
        rssis = [label_best.get(lbl, {}).get("RSSI") for lbl in ap_labels]
        if any(v is None for v in rssis):
            continue
        tdb = _total_power_db(rssis)
        if tdb is None or not math.isfinite(tdb):
            continue
        for i, lbl in enumerate(ap_labels):
            y = float(rssis[i])
            onehot = [1.0 if j == i else 0.0 for j in range(len(ap_labels))]
            rows.append(onehot + [tdb])
            ys.append(y)

    # Per-AP near-baseline (95th percentile)
    ap_p95 = {}
    for lbl in ap_labels:
        vals = per_ap_values.get(lbl, [])
        ap_p95[lbl] = float(np.percentile(vals, 95.0)) if len(vals) > 0 else 0.0
    ...
    if rows:
        ...
        ap_intercepts = {ap_labels[i]: float(coef[i]) for i in range(len(ap_labels))}
        gamma = float(coef[-1])
    else:
        # Fallbacks
        ap_intercepts = {lbl: (float(np.mean(per_ap_values.get(lbl, [0.0]))) if per_ap_values.get(lbl) else 0.0)
                         for lbl in ap_labels}
        gamma = 0.0

    return {
        "ap_p95": ap_p95,
        "ap_intercepts": ap_intercepts,
        "gamma": gamma,
    }
...
def softmax_shares(rssis: Sequence[Optional[float]]):
...
def transform_wifi_data(db,
    ...
    apname_to_label = {ap_name: f"{ap_name}{AP_SUFFIX}" for ap_name in ap_positions.keys()}
    label_to_ap_pos = {}
    ...
    # Aggregate per-AP stats (mean and mode channel) for imputation
    label_sum = defaultdict(float)
    label_count = defaultdict(int)
    label_channel_counter = defaultdict(Counter)

    pre_docs = []  # (raw_doc, label_best)
    for doc in raw_docs:
        if isinstance(doc.get("data"), dict) and "error" in doc["data"]:
            stats["error_docs"] += 1
            continue

        # Pick best RSSI per BSSID and then best per AP label
        bssid_to_best = {}
    ...
    # --------------------------------------------------
    # Fit LDPL (legacy) using x,y (kept for backwards compatibility)
    # --------------------------------------------------
    X_rows = []
    y_vals = []
    ...
    ap_intercepts_ldpl = {l: float(beta[i]) for i, l in enumerate(ap_labels)}
    beta1 = float(beta[-2])
    beta2 = float(beta[-1])
    n_est = -beta1 / 10.0 if beta1 != 0 else None

    if debug:
        print("LDPL fit: ap_intercepts:", {k: round(v, 2) for k, v in ap_intercepts_ldpl.items()})
        print(f"LDPL fit: beta1 (log10 d)={beta1:.3f}, beta2 (20log10 f)={beta2:.3f}, n_est≈{n_est:.3f}")

    # --------------------------------------------------
    # NEW: Fit x,y-free calibration used for rssi_1m & residual features
    # --------------------------------------------------
    xyfree_calib = fit_xy_free_calibration(pre_docs, ap_labels)
    if debug:
        print("x,y-free ap_p95:", {k: round(v, 2) for k, v in xyfree_calib["ap_p95"].items()})
        print("x,y-free ap_intercepts:", {k: round(v, 2) for k, v in xyfree_calib["ap_intercepts"].items()})
        print(f"x,y-free gamma: {xyfree_calib['gamma']:.4f}")
    ...
    for d, label_best in pre_docs:
        try:
            ...
            # Build per-doc RSSI & Channel dicts, with imputation where necessary
            ...
            # ----------------------------
            # x,y-free feature computation
            # ----------------------------
            tdb = _total_power_db([ap_rssi[lbl] for lbl in ap_labels])
            tdb = tdb if (tdb is not None and math.isfinite(tdb)) else 0.0
            ...
            for label in ap_labels:
                ...
                if rssi is None:
                    ...
                    rssi_1m[label] = None
                    continue
                # rssi_1m-like normalization via per-AP near baseline
                rssi_1m[label] = float(rssi) - float(xyfree_calib["ap_p95"][label])
                # residual vs. RSSI-only model
                pred = float(xyfree_calib["ap_intercepts"][label]) + float(xyfree_calib["gamma"]) * tdb
                residuals[label] = float(rssi) - pred

            # The existing bias-corrected deltas remain based on LDPL intercepts (for backward compat)
            def bias_correct(v, lbl):
                if v is None:
                    return None
                return v - ap_intercepts_ldpl.get(lbl, 0.0)
            ...
            new_doc = {
                ...
                **ratio_cols,
                **power_ratio_cols,
                # x,y-free versions (same names, different definitions)
                **{f"{lbl}_residual": residuals[lbl] for lbl in ap_labels},
                **{f"{lbl}_rssi_1m": rssi_1m[lbl] for lbl in ap_labels},
                # Keep legacy LDPL fit outputs for debugging / compatibility
                **{f"ap_intercepts_{lbl}": ap_intercepts_ldpl[lbl] for lbl in ap_labels},
                "beta1_log10d": beta1,
                "beta2_20log10f": beta2,
                "n_est": n_est,
                # Also export x,y-free calibration params in case you want them
                **{f"xyfree_intercept_{lbl}": xyfree_calib["ap_intercepts"][lbl] for lbl in ap_labels},
                "xyfree_gamma_total_power_db": xyfree_calib["gamma"],
            }
...
if __name__ == "__main__":
    ...
    flat_ap_mapping = {
        bssid.lower(): f"{ap_name}{AP_SUFFIX}"
    ...
    for triangle_name, current_triangle in triangle_dictionary.items():
        ...
        output_db = client["wifi_fingerprinting_data_extra_features_no_leak"]
